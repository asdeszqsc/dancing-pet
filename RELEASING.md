...
Apple Developer ID 없음(**미공증**) — Homebrew cask 가 quarantine 속성을 제거해 경고 없이 실행. 직접 `.dmg` 배포 시 Gatekeeper 경고가 날 수 있음.

대신 **ad-hoc 서명 + 고정 designated requirement** 를 쓴다
(`tauri/scripts/macos-adhoc-sign.sh` — `tauri.macos.conf.json` 의 `beforeBundleCommand` 로 번들 직전 실행):

- 문제: 링커 기본 ad-hoc 서명은 identifier 가 빌드마다 랜덤(`tauri-<hex>`)이고,
  TCC(손쉬운 사용)는 권한 부여 시점의 designated requirement(ad-hoc 기본값: **cdhash** = 그 바이너리 해시)를
  저장한다 → `brew upgrade` 마다 권한이 조용히 무효화 (설정엔 켜져 있는데
  `AXIsProcessTrusted()` = false → Dock 감지 불가).
- 해결: identifier 를 `com.dancingpet.app` 으로 고정 + designated requirement 를
  `identifier "com.dancingpet.app"` (cdhash 없음)로 심음 → 이후 빌드가 기존 권한 항목과 계속 매칭.
- CI 의 **Verify macOS signature** 스텝이 릴리스 빌드의 requirement 를 검증한다.
- ⚠️ 트레이드오프: ad-hoc 이라 같은 identifier 를 주장하는 임의의 무서명 앱도 이 requirement 를
  만족할 수 있음. Developer ID 도입 시 스크립트를 제거하고 `bundle.macOS.signingIdentity` 로 대체할 것.
- **기존 사용자 1회 마이그레이션 필요**: 이 방식 도입 이전 버전에서 부여한 권한은 cdhash 기준이라
  새 빌드와 매칭 안 됨 → `tccutil reset Accessibility com.dancingpet.app` 후 앱 재실행·재허용 1회
  (이후 업데이트부터는 유지됨). cask caveats 에도 안내되어 있음.
